...
class ChatService {
  ...
  public async getUserConversationList(userId: ObjectId): Promise<IMessageData[]> {
    const messages: IMessageData[] = await MessageModel.aggregate([
      ...
      {
        $group: {
          _id: '$conversationId', // group all the documents by conversationId and set the property as _id
          result: { $last: '$$ROOT' } // get the last item from the group and set the document as the result
        }
      },
      {
        $project: {
