...
class UserService {
  ...
  public async getUserByAuthId(authId: string): Promise<IUserDocument> {
    const users: IUserDocument[] = await UserModel.aggregate([
      { $match: { authId: new mongoose.Types.ObjectId(authId) } }, // we pass authId as a string, so we need to cast it into a mongoose.Types.ObjectId
      { $lookup: { from: 'Auth', localField: 'authId', foreignField: '_id', as: 'authId' } }, // go to Auth collection, the field is authId coming from userSchema, the field is _id in Auth collection, want to return the result with a key called authId(whatever you want)
      { $unwind: '$authId' }, // lookup returns its results inside a list, but it will return its results as an object by using unwind operator.
      { $project: this.aggregateProject() } // call project to return the data we want
    ]);
    return users[0];
  }
