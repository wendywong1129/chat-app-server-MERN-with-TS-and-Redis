...
// import { commentNames } from './../../../mocks/reactions.mock';
...
class CommentService {
  ...
  public async getPostCommentNames(query: IQueryComment, sort: Record<string, 1 | -1>): Promise<ICommentNameList[]> {
    const commentsNamesList: ICommentNameList[] = await CommentsModel.aggregate([
      { $match: query },
      { $sort: sort },
      { $group: { _id: null, names: { $addToSet: '$username' }, count: { $sum: 1 } } }, // use group operator to group data by keys(names、count)
      { $project: { _id: 0 } } // use project operator to remove or keep setting field
    ]);
